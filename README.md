...
| 역할       | 이름     | 설명                                      |
|------------|----------|-------------------------------------------|
| PM (Project Manager) | 함선우   | 프로젝트 전반을 관리하고 팀을 이끄는 역할을 맡고 있습니다. |
| TL (Tech Leader) | 김동욱   | 기술적 리더십을 제공하며 주요 기술적 문제를 해결합니다. |
| GM (Geometry Manager) | 임영인   | 시스템 구조와 데이터 관리를 책임지며, 전체 아키텍처를 설계합니다. |
| AC (Agile Coach) | 김도현   | 애자일 방법론을 적용하여 팀의 작업 효율성을 높이고, 프로젝트 관리를 지원합니다. |
